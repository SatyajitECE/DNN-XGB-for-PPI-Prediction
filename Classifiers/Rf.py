...
for train, test in kf.split(X,y):
    o=o+1
    cv_clf = RandomForestClassifier(n_estimators=500)
...
Rf_fpr=pd.DataFrame(Rf_fpr)
Rf_tpr=pd.DataFrame(Rf_tpr)

Rf_fpr.to_csv('fpr_Rf.csv',header=False, index=False)
Rf_tpr.to_csv('tpr_Rf.csv',header=False, index=False)   
