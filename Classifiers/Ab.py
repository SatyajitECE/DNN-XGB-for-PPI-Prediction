...
for train, test in kf.split(X,y):
    ...
    recall.append(rec)
    Ab_fpr_list.append(fpr)
    Ab_tpr_list.append(tpr)
    if max_accuracy<acc:
        max_accuracy=acc
        Ab_fpr=fpr
        Ab_tpr=tpr
...
Ab_fpr=pd.DataFrame(Ab_fpr)
Ab_tpr=pd.DataFrame(Ab_tpr)

Ab_fpr.to_csv('fpr_Ab.csv',header=False, index=False)
Ab_tpr.to_csv('tpr_Ab.csv',header=False, index=False)   
